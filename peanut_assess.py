...
def visualize_instances_with_colors(image, masks):
    ...
    np.random.seed(42)
    colors = [np.random.randint(0, 255, size=3, dtype=np.uint8) for _ in range(len(masks))]
        ...
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            for result in results:
                if hasattr(result, 'masks') and result.masks is not None:
                    ...
                    for mask, bbox in zip(adjusted_masks, adjusted_bboxes):
                        x_min, y_min, x_max, y_max = bbox
                        cv2.rectangle(vis_image, (x_min, y_min), (x_max, y_max), (255, 0, 0), 2)
                        vis_image[mask > 0] = (0, 255, 0)  # Green for mask
...
def display_model_summary(model):
    # Assuming your model is compatible with a 3x640x640 input
    summary(model.model, input_size=(3, 640, 640))


if __name__ == "__main__":
    # choice = input("Enter 'train' to train the model or 'analyze' to process images: ").strip().lower()
    # if choice == 'train':
    #     train_model()
    # elif choice == 'analyze':
    #     analyze_images()
